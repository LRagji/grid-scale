...
import { IPageInfo, RDriver, RPage } from "../../src/index.js";
import { PageRankedElement } from "../../src/redis-wal/r-page.js";
...
function dumpPageElementsToSqlite(pageInfo: IPageInfo, pageKey: string, pageElements: Map<string, Map<number, PageRankedElement>>, tempDir = tmpdir()): string {
    ...
    try {
        ...
        for (const [groupKey, groupedElements] of pageElements.entries()) {
            for (const element of groupedElements.values()) {
                insertStatement.run(
                    groupKey,
                    element.elementRank,
                    element.sn,
...
async function checkpointHandler(job: Job<IPageInfo>, token?: string, abortSignal?: AbortSignal): Promise<void> {
    ...
    const page = new RPage(driver, job.data.pageKey, undefined, timeToleranceInMs * 2);
    const pageElements = await page.dumpDataFromPage();
