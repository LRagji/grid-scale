...
// export type TSampleSet = [tagName: string, number[]][];
...
// export function writeData(selfId: string, config: TConfig, sampleSet: TSampleSet, preConditionedTables: Set<string>, systemTime = Date.now()): IWriterOutput {
//     const chunks = new Map<string, IChunkInfo>();
//     const dataDisplacements = new Map<string, Set<string>>();
//     const diskPaths = config.setPaths.get(config.activePath) || ["./default-data/"];

//     for (let payloadIndex = 0; payloadIndex < sampleSet.length; payloadIndex += 1) {
//         const tagName = sampleSet[payloadIndex][0];
//         const samples = sampleSet[payloadIndex][1];

//         const chunkId = Calculators[config.activeCalculatorIndex](tagName, systemTime, config.tagBucketWidth, config.timeBucketWidth);
...
//         const dbPath = join(diskPath, logicalChunkId, `${config.fileNamePre}${selfId}${config.fileNamePost}`);
...
//                 const sampleBucketedTime = sampleTime - (sampleTime % config.timeBucketWidth);
//                 if (sampleBucketedTime < (chunkId.timePart - (config.timeBucketWidth * config.timeBucketTolerance)) || sampleBucketedTime > (chunkId.timePart + (config.timeBucketWidth * config.timeBucketTolerance))) {
...
//                     const misplacedLogicalChunkId = misplacedChunkId.logicalChunkId(config.logicalChunkPrefix, config.logicalChunkSeperator);
